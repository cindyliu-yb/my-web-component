...
function validateUserBundles(bundles) {
    ...
    bundles.forEach(function (b) {
        ...
        b.components = b.components.map(function (tag) {
...
function validateComponentTag(tag, suffix) {
...
function getElementDecoratorMeta(fileMeta, classNode) {
...
function getEventDecoratorMeta(moduleFile, diagnostics, classNode) {
...
function validateEvent(fileMeta, rawEventOpts, methodName, memberNode) {
...
function getListenDecoratorMeta(moduleFile, diagnostics, classNode) {
    ...
    decoratedMembers.forEach(function (memberNode) {
        ...
        if (isListen && eventName && methodName) {
            eventName.split(',').forEach(function (evName) {
                var listenMeta = validateListener(moduleFile, evName, rawListenOpts, methodName);
                if (listenMeta) {
    ...
    moduleFile.cmpMeta.listenersMeta = moduleFile.cmpMeta.listenersMeta.sort(function (a, b) {
...
function validateListener(fileMeta, eventName, rawListenOpts, methodName) {
...
function getMethodDecoratorMeta(fileMeta, classNode) {
    var decoratedMembers = classNode.members.filter(function (n) { return n.decorators && n.decorators.length; });
    var methodMemebers = decoratedMembers.filter(function (n) { return n.kind === ts.SyntaxKind.MethodDeclaration; });
    methodMemebers.forEach(function (methodNode) {
        var isMethod = false;
        var methodName = null;
...
function getPropDecoratorMeta(moduleFile, diagnostics, classNode) {
    var decoratedMembers = classNode.members.filter(function (n) { return n.decorators && n.decorators.length; });
    decoratedMembers.forEach(function (memberNode) {
        var isProp = false;
        var propName = null;
        ...
        if (isProp && propName) {
            ...
            var propMeta = moduleFile.cmpMeta.membersMeta[propName] = {
...
function getPropChangeDecoratorMeta(fileMeta, classNode) {
...
function getPropChangeDecorator(classNode, decoratorName, propChangeMeta) {
    var decoratedMembers = classNode.members.filter(function (n) { return n.decorators && n.decorators.length; });
    decoratedMembers.forEach(function (memberNode) {
    ...
    propChangeMeta = propChangeMeta.sort(function (a, b) {
...
function getStateDecoratorMeta(fileMeta, classNode) {
    var decoratedMembers = classNode.members.filter(function (n) { return n.decorators && n.decorators.length; });
    decoratedMembers.forEach(function (memberNode) {
        var isState = false;
        var propName = null;
...
function removeClassDecorator(classNode) {
    return ts.createClassDeclaration(undefined, // <-- that's what's removing the decorator
    // everything else should be the same
    classNode.modifiers, classNode.name, classNode.typeParameters, classNode.heritageClauses, classNode.members);
}
...
function componentClass(config, moduleFiles, diagnostics) {
    return function (transformContext) {
        function visitClass(moduleFile, classNode) {
        ...
        function visit(fileMeta, node) {
            switch (node.kind) {
                case ts.SyntaxKind.ClassDeclaration:
                    return visitClass(fileMeta, node);
                default:
...
function getUserTsConfig(config, ctx) {
    if (ctx.tsConfig) {
        return ctx.tsConfig;
    }
    // // force defaults
    var options = Object.assign({}, DEFAULT_COMPILER_OPTIONS);
...
function updateLifecycleMethods() {
    return function (transformContext) {
...
function transpileSync(config, ctx, moduleFiles) {
...
function transpileModules(config, ctx, moduleFiles, transpileResults) {
    ...
    var tsOptions = getUserTsConfig(config, ctx);
...
function compileFileSync(config, ctx, filePath) {
    var moduleFiles = {};
    moduleFiles[filePath] = getModuleFileSync(config, ctx, filePath);
    var transpileResults = transpileSync(config, ctx, moduleFiles);
    var moduleFile = transpileResults.moduleFiles[filePath];
...
function mockPlatform() {
    ...
    var win = config.sys.createDom().parse({ html: '' });
    var domApi = createDomApi(win.document);
    var plt = createPlatformServer(config, win, win.document, diagnostics, false);
    plt.isClient = true;
    var $mockedQueue = plt.queue = mockQueue();
    var $loadBundleQueue = mockQueue();
    plt.loadBundle = function (a, elm, cb) {
        a;
        elm;
        $loadBundleQueue.add(cb);
    };
    plt.$flushQueue = function (cb) {
        $mockedQueue.flush(cb);
    };
    plt.$flushLoadBundle = function (cb) {
        $loadBundleQueue.flush(cb);
    };
    var renderer = createRenderer(plt, domApi);
    plt.render = function (oldVNode, newVNode, isUpdate, hostElementContentNode) {
        return renderer(oldVNode, newVNode, isUpdate, hostElementContentNode);
    };
    return plt;
}
function mockBuildConfig() {
...
function mockFs() {
    ...
    var orgreadFileSync = fs.readFileSync;
    var orgwriteFileSync = fs.writeFileSync;
...
function mockConnect(plt, html) {
    var rootNode = jsdom.JSDOM.fragment(html);
    connectComponents$1(plt, rootNode);
    return rootNode;
}
function connectComponents$1(plt, node) {
    if (!node)
        return;
    if (node.tagName) {
        if (!node._hasConnected) {
            var cmpMeta = plt.getComponentMeta(node);
            if (cmpMeta) {
                initHostConstructor(plt, node);
    ...
    if (node.childNodes) {
        for (var i = 0; i < node.childNodes.length; i++) {
            connectComponents$1(plt, node.childNodes[i]);
        }
    }
}


function removeWhitespaceFromNodes(node) {
    if (node.nodeType === 1) {
...
function register(components) {
    ...
    if (components) {
        components.forEach(function (c) { return mockDefine(testPlatform, c); });
    }
}
function render(html) {
    var node = mockConnect(testPlatform, html);
    return waitForLoad(node);
}
function transpile(tsFileName, srcDir) {
    var config = mockBuildConfig();
...
function waitForLoad(rootNode) {
    return new Promise(function (resolve) {
        testPlatform.$flushQueue(function () {
            // flush to read attribute mode on host elment
            testPlatform.$flushLoadBundle(function () {
...
function connectComponents(node) {
    if (!node)
        return;
    if (node.tagName) {
        if (!node._hasConnected) {
            var cmpMeta = testPlatform.getComponentMeta(node);
            if (cmpMeta) {
                initHostConstructor(testPlatform, node);
                node.connectedCallback();
            }
        }
    }
    if (node.childNodes) {
        for (var i = 0; i < node.childNodes.length; i++) {
            connectComponents(node.childNodes[i]);
        }
    }
}
