...
var WEB_DEV_EXT = ['ts', 'tsx', 'js', 'jsx', 'html', 'htm', 'css', 'scss', 'sass'];
...
function readCssFile(config, ctx, absStylePath, styleCollection) {
    ...
    var sys = config.sys;
    return readFile(sys, absStylePath).then(function (cssText) {
...
function getModuleFile(config, ctx, tsFilePath) {
    ...
    if (moduleFile) {
        ...
        return readFile(config.sys, tsFilePath).then(function (tsText) {
...
function generateGlobalJs(config, globalJsContents) {
    var content = "/* " + config.namespace + " Global is used during Server-Side Rendering and Prerendering */\n";
...
function transpile(config, ctx, moduleFiles) {
...
function copySourceSassFilesToDest(config, ctx, compileResults) {
    ...
    return Promise.all(compileResults.includedSassFiles.map(function (sassSrcPath) {
        ...
        return readFile(sys, sassSrcPath).then(function (sassSrcText) {
...
function generateHtmlDiagnostics(config, diagnostics) {
    // TODO
    config;
    diagnostics;
    generateHtmlDocument;
}
function generateHtmlDocument(diagnostics) {
...
function generateDiagnosticHtml(d) {
    ...
    var title = toTitleCase(d.type) + " " + toTitleCase(d.level);
...
function generateIndexHtml(config, ctx) {
    ...
    return readFile(config.sys, config.srcIndexHtml).then(function (indexSrcHtml) {
...
function loadDependentManifest(config, ctx, dependentCollection) {
    ...
    return readFile(config.sys, dependentManifestFilePath).then(function (dependentManifestJson) {
 ...
 */
function createRenderer$1(plt, domApi) {
...
function createPlatformServer(config, win, doc, diagnostics, isPrerender, ctx) {
    ...
    plt.render = createRenderer$1(plt, domApi);
...
function hydrateHtml(config, ctx, registry, opts, hydrateResults, callback) {
    ...
    plt.onAppLoad = function (rootElm, stylesMap) {
...
function createRenderer(config, registry, ctx) {
    ...
    function hydrateToString(opts, callback) {
        var hydrateResults = {
            diagnostics: [],
            html: opts.html,
            styles: null,
...
function validateHydrateOptions(config, opts) {
    var req = opts.req;
...
function validateRendererConfig(config) {
    ...
    if (!config.logger && require) {
        ...
        var path = require('path');
        var logger = require(path.join(__dirname, '../cli/util')).logger;
...
function isConfigValid(config, diagnostics) {
    try {
        // validate the build config
        validateBuildConfig(config);
    }
